...
export default function PendingInvite({ user, profile }) {
  const [accepting, setAccepting] = useState(false)
  const [declining, setDeclining] = useState(false)
  const invite = profile.pendingInviteFrom // { uid, name, email, listId }
  ...
  async function handleAccept() {
    ...
    try {
      ...
      if (listId) {
        // Add recipient to sender's existing list
      ...
      // Update sender's profile (cross-user — allowed by rules)
  ...
  async function handleDecline() {
    setDeclining(true)
    try {
      // Clear own pending invite
      await updateDoc(doc(db, 'users', user.uid), { pendingInviteFrom: null })

      // Clear sender's pendingInviteTo so they can send again (cross-user)
...
const s = {
  ...
  desc: {
    color: 'var(--navy-mid)', fontSize: '15px',
